...
class DependencyModel(Module): 
  ...
  def __init__(self, word_types, outputs):
    ...
    self.hidden1 = Linear(in_features=6*128, out_features=128)
    self.hidden2 = Linear(in_features=128, out_features=128)
    self.output = Linear(in_features=128, out_features=91) 

  def forward(self, inputs):
    embedded = self.embedding(inputs)
    flattened = embedded.view(embedded.shape[0], -1)
    hidden_output1 = relu(self.hidden1(flattened))
    hidden_output2 = relu(self.hidden2(hidden_output1))
