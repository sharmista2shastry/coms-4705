...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        def get_word_representation(word_list, pos_list, index):
            if word_list[index] is None:
                return self.word_vocab['<ROOT>']
            elif pos_list[index] == 'CD':
                return self.word_vocab['<CD>']
            elif pos_list[index] == 'NNP':
                return self.word_vocab['<NNP>']
            elif pos_list[index] == 'UNK':
                return self.word_vocab['<UNK>']
            elif not self.word_vocab.get(word_list[index].lower()):
                return self.word_vocab['<UNK>']
            else:
                return self.word_vocab[word_list[index].lower()]
        ...
        for i in range(1, 4):
            if len(state.stack) >= i:
                input_list.append(get_word_representation(words, pos, state.stack[-i]))
            else:
        ...
        for i in range(1, 4):
            if len(state.buffer) >= i:
                input_list.append(get_word_representation(words, pos, state.buffer[-i]))
            else:
                input_list.append(self.word_vocab['<NULL>'])
    ...
    def get_output_representation(self, output_pair):
        output_list = np.zeros(91)  # Initialize a numpy array with zeros for all possible output labels
        
        # Set the index corresponding to the given output pair to 1
        output_index = self.output_labels.get(output_pair)
        if output_index is not None:
            output_list[output_index] = 1
