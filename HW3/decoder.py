...
class Parser(object):
    ...
    def parse_sentence(self, words, pos):
        ...
        while state.buffer:
            ...
            with torch.no_grad():
                feature_tensor = torch.from_numpy(np.array([feature,])).long()
                pred = self.model(feature_tensor)
            indices = np.argsort(-pred, axis=1)[0]

            for i in indices:
                action, label = self.output_labels[i.item()]
                if action == 'shift' and state.buffer:
                    state.shift()
                    break
                elif action == 'left_arc' and state.stack and label != 'root':
                    state.left_arc(label)
                    break
                elif action == 'right_arc' and state.stack:
